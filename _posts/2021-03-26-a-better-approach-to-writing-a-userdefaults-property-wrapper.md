...
Related to the "default value problem" is the use of optionals. Most developers agree that we should try to [eliminate optionals in Swift as much as possible]({% post_url_absolute 2015-04-06-swift-failable-initializers-revisited %}). If you provide a default value using `register(defaults:)`, then retrieving it will *always* return *something* (i.e., a non-optional value). This means that in many cases we can be sure a value exists in `UserDefaults`.
